...
df_diversity  <- read.csv(r"(G:\Bio-Analysis\Python\Vscode\Task01-NEW\Data_submission\GitHub_repository\Data_repository\Supplementary_figures\alpha_diversity-comparison-pneumonia_vs_no_pneumonia.csv)",header = TRUE)
...
df_vf_num = read.csv(r"(LRT_microbiomes_critical_illness\Data_repository\Main_figures\vfdb_profile-species_all-vf_num.csv)")
...
df_argtypes_collection <- read.csv(r"(LRT_microbiomes_critical_illness\Data_repository\Supplementary_figures\argtypes-collection_all_sites.csv)")
...
df_loading1 <- read.csv(r'(LRT_microbiomes_critical_illness\Data_repository\Supplementary_figures\PLS-DA_arg_subtypes-all_sites-loadings-comp1.csv)',header = TRUE)
...
df_loading2 <- read.csv(r'(LRT_microbiomes_critical_illness\Data_repository\Supplementary_figures\PLS-DA_arg_subtypes-all_sites-loadings-comp2.csv)',header = TRUE)
...
df_mge_abundance = read.csv(r"(\mobile_elements_finder-stats_comparisons-mge_abundance.csv)")
...
# Sample dataset
nodes <- read.csv(r'(LRT_microbiomes_critical_illness\Data_repository\Main_figures\mobile_elements_finder-draft_genomes-arg_sankey-nodes.csv)',header = F)
names(nodes) <- c('name')
links <- read.csv(r'(LRT_microbiomes_critical_illness\Data_repository\Main_figures\mobile_elements_finder-draft_genomes-arg_sankey-links-scaled.csv)')
...
#replace patient name with patientID
df_plasmids_ani$query <- str_replace(df_plasmids_ani$query,'zhujianfen-zheyi','Patient_6')
colnames(df_plasmids_ani) <-  str_replace(colnames(df_plasmids_ani),'zhujianfen.zheyi','Patient_6')
#remove suffix: '.fasta' 
...
bbone <- read_mauve_backbone(r'(E:\learning materials\graduate student\Lab-Data\Task-01-New\Processed-Data\MGEs\PLSDB\mauve_alignment\plasmids_publish_selected.backbone)',
                             ref=1, filter_low=0)
...
df_recombinations_all <- read.csv(r'(LRT_microbiomes_critical_illness\Data_repository\Main_figures\acinetobacter-recombinations-consensus_blocks_all.csv)')
df_pi <- read.table(r"(LRT_microbiomes_critical_illness\Data_repository\Main_figures\all_patients-acinetobacter.pi)",header = T)
...
df_tree <- read.tree(r"(LRT_microbiomes_critical_illness\Data_repository\Main_figures\draft_only-outgroup_removed-snpscalling_filtered_resolved.tre)")
df_tree2 <- read.tree(r"(LRT_microbiomes_critical_illness\Data_repository\Main_figures\draft_only-outgroup_removed-snpscalling_filtered_resolved.tre2)")
df_meta <- read.csv(r"(LRT_microbiomes_critical_illness\Data_repository\Main_figures\metadata-filename2sampleid.csv)")
...
### Fig s5g
bbone <- read_mauve_backbone(r'(LRT_microbiomes_critical_illness\Data_repository\Supplementary_figures\plasmids_publish_selected.backbone)',
                             ref=1, filter_low=0)
names <- c('NZ_CP010782.1',
           'Patient_6')
names(bbone$dna_segs) <- names
...
### Fig s5g
bbone <- read_mauve_backbone(r'(LRT_microbiomes_critical_illness\Data_repository\Supplementary_figures\plasmids_selected-comparisons-NZ_CP045266.1.backbone)',
                             ref=1, filter_low=0)
...
#replace patient name with patientID
df_plasmids_ani$query <- str_replace(df_plasmids_ani$query,'zhouboxun-xiangya','Patient_115')
colnames(df_plasmids_ani) <-  str_replace(colnames(df_plasmids_ani),'zhouboxun.xiangya','Patient_115')
#remove suffix: '.fasta' 
colnames(df_plasmids_ani) <- str_remove(colnames(df_plasmids_ani),'.fasta')
row.names(df_plasmids_ani) <- str_remove(df_plasmids_ani$query,'.fasta')
